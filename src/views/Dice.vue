<template>
  <div class="dice-container">
    ...
    <div class="controls">
      <el-button type="primary" @click="rollAllDice" :disabled="isRolling">摇骰子</el-button>
      <el-button @click="addDie">添加骰子</el-button>
      <el-button @click="removeDie" :disabled="dice.length <= 1">移除骰子</el-button>
...
// Three.js 相关
let scene, camera, renderer, diceMeshes = [], diceBodies = []
...
// 总点数
const total = computed(() => dice.value.reduce((sum, die) => sum + die.value, 0))
...
function initCannonJS() {
  ...
  const wallFront = new CANNON.Body({
    mass: 0,
    material: groundMaterial,
    shape: new CANNON.Box(new CANNON.Vec3(arenaSize / 2, wallHeight / 2, wallThickness / 2))
  })
  ...
  const wallLeft = new CANNON.Body({
    mass: 0,
    material: groundMaterial,
    shape: new CANNON.Box(new CANNON.Vec3(wallThickness / 2, wallHeight / 2, arenaSize / 2))
  })
...
function createDiceMesh() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    dots.forEach(([x, y]) => {
      ...
      // 点的颜色
      const dotColor = isRed ? ['#c0392b', '#921e1e', '#5a0f0f'] : ['#2c3e50', '#1a252f', '#0d1419']
      const glowColor = isRed ? 'rgba(192, 57, 43, 0.2)' : 'rgba(0, 0, 0, 0.1)'

      // 点的外发光效果
      const dotGradient = context.createRadialGradient(dotX, dotY, 0, dotX, dotY, dotRadius * 1.5)
...
function calculateDiceValue(body) {
  ...
  for (let i = 0; i < localNormals.length; i++) {
    // 将局部法向量旋转到世界坐标系
    const worldNormal = localNormals[i].clone().applyQuaternion(body.quaternion)
